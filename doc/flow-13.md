...
Als er documenten met betrekking op de BPV van een student in het DMS gecreëerd of opgeslagen worden die niet afkomstig zijn van de MORA bpv module dan kan het DMS deze documenten aanmelden bij de module zodat deze getoond en opgevraagd kunnen worden. De inhoud van het document wordt niet uitgewisseld. 
...
#### Opmerking: we zoeken nog of we voldoende informatie hebben om een stage te kunnen identificeren. Op ingescande documenten staat dat niet (geen uuid's). Huidige idee is om de documenten te plaatsen in een actieve bpv afhankelijk van de scan datum (receivedDate).
